...
## Highlights:
   ...
   * Raspberry Pi support  **both with and without hardware  video decoding** by the
   Broadcom GPU.  _Tested on Raspberry Pi 5, Pi 4 Model B and  Pi 3 model B+._
   * Support for running on Microsoft Windows (builds with the MinGW-64 compiler in the
     unix-like MSYS2 environment).

   * _NEW in v1.67: support for one-time Apple-style "pin" code client authentication ("client-server 
...
1.67 2023-11-30   Add support for Apple-style one-time pin authentication of clients with option "-pin":
                  (SRP6a authentication protocol) and public key persistence.   Detection with error message
		  ...
		  specify a replacement MAC address/Device ID.
