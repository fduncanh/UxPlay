...
-   Support for Raspberry Pi, with hardware video acceleration using the
    ...
    UxPlay viable on other distributions of Raspberry Pi that do not
...
-   **Video4Linux2 support for the Raspberry Pi Broadcom 2835 GPU**
    ...
    Raspberry Pi (RPi) computers (tested on model 4 B) can now run
    UxPlay using software decoding of h264 video, but
    hardware-accelerated decoding by firmware in the Pi's GPU is
    prefered. UxPlay can access the GPU using the GStreamer Video4Linux2
    (v4l2) plugin (this replaces access with OpenMax, which is 32-bit
    only and no longer maintained, used in UxPlay's ancestor RPiPlay).
    The recent GStreamer-1.22 v4l2 plugin works well with UxPlay:
    backports have been made to GStreamer-1.18.4 in Raspberry Pi OS, and
    patches to this and later v1.20.x GStreamer are available in the
    UxPlay Wiki (see [patching instructions for
    ...
    in included in Raspberry Pi OS and other distributions (Ubuntu,
    Manjaro) available with Raspberry Pi Imager.
...
in the build directory after the build process, if you wish to test
