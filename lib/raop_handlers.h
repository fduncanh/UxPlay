                  ...
                  char **response_data, int *response_datalen)
{
    ...
    plist_t features_ex_node = plist_new_string("1c9/St5PFbgmIQQ");
    plist_dict_set_item(res_node, "featuresEx", features_ex_node);
    ...
    plist_t displays_0_max_width_pixels_node = plist_new_uint(3840);
    plist_t displays_0_height_pixels_node = plist_new_uint(conn->raop->height);
    plist_t displays_0_max_height_pixels_node = plist_new_uint(2160);
    ...
    plist_dict_set_item(displays_0_node, "widthPixelsMax", displays_0_max_width_pixels_node);
    plist_dict_set_item(displays_0_node, "heightPixels", displays_0_height_pixels_node);
    plist_dict_set_item(displays_0_node, "heightPixelsMax", displays_0_max_height_pixels_node);
    ...
    plist_dict_set_item(displays_0_node, "receiverSupports444", displays_0_supports444);
    // plist_dict_set_item(displays_0_edid, "edid", displays_0_features);
                       ...
                       char **response_data, int *response_datalen)
{
    ...
    if (datalen == 32) {
        logger_log(conn->raop->logger, LOGGER_DEBUG, "PAIRING: Legacy pairing");
        ...
        *response_data = malloc(sizeof(public_key));
        ...
        switch (data[2]) {
            case 0x00: // M1 (usually 0x01, 0x06, 0x00, 0x01)
                const char *salt;
                // char pin[6];
                const char *pk;
                int len_pk, len_salt;
                char *method = NULL;
                // char *user = NULL;
                int ret = srp_new_user(conn->session, conn->raop->pairing, "Pair-Setup",
                               "3939", &salt, &len_salt, &pk, &len_pk);
                *response_data = malloc(409);
                char *res_data = *response_data;
                *response_datalen = 409;
